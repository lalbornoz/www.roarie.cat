...
},{"./src/canvas":2,"./src/color":3,"./src/font":4,"./src/style":9,"./src/to_json":10}],2:[function(require,module,exports) {
...
var render_colorcode = function(json, canvas, font, opts) {
  ...
  canvas_tmp.width = cw; canvas_tmp.height = ch;
  canvas.width = json.w * cw; canvas.height = json.h * ch;

  // pre fill entire canvas with bg color; is this a good optimization?
  ...
  for (var l=0; l<json.lines.length; l++) {
    ...
    for (var c=0; c<line.length; c++) {
      var char = line[c]; var x = c * cw; var y = l * ch
      ...
      if (char.u) {
        ctx.beginPath()
        ctx.strokeStyle = palette[char.fg]
        ctx.lineWidth = 1
        ctx.moveTo(x, y + ch)
      ...
      if (fg !== color.transparent) {
        ...
        ctx_tmp.fillStyle = fg; ctx_tmp.fillRect(0,0,cw,ch)
...
var to_canvas = function(string_or_json, opts) {
  opts = opts || {};
  if (typeof string_or_json === 'string') string_or_json = to_json(string_or_json, opts);
...
style.palette = 'mirc'; color.transparent_index = 99; color.transparent = 'rgba(0,0,0,0)';
...
},{"./font/fixedsys":6,"./style":9,"canvas":11}],6:[function(require,module,exports) {
...
var style_chars = {'\x02': 'bold', '\x16': 'inverse', '\x1d': 'italic', '\x0f': 'reset', '\x1f': 'underline'};
var style_fns = {};

// {{{ style_fns.bold = function(style)
style_fns.bold = function(style) { style.b = !style.b };
// }}}
// {{{ style_fns.inverse = function(style)
style_fns.inverse = function(style) { var tmp = style.fg; style.fg = style.bg; style.bg = tmp; };
// }}}
// {{{ style_fns.italic = function(style)
style_fns.italic = function(style) { style.i = !style.i };
// }}}
// {{{ style_fns.underline = function(style)
style_fns.underline = function(style) { style.u = !style.u };
// }}}
// {{{ style_fns.reset = function(style, base_style)
style_fns.reset = function(style, base_style) {
  style.b = base_style.b; style.i = base_style.i; style.u = base_style.u;
  style.bg = base_style.bg; style.fg = base_style.fg;
};
// }}}
...
var Style = function(style) {
  this.b = style.b; this.i = style.i; this.u = style.u;
  this.bg = style.bg; this.fg = style.fg;
};
// }}}

colorcode_to_json.defaults = { b: false, i: false, u: false, fg: 1, bg: 99 };
